...
| **Extra** (attached to a delivery)  | Runs awarded outside the delivery's off-the-bat total.                                                                     | extras type (wide / no-ball / bye / leg bye / penalty — nullable columns, since types co-occur, e.g. a wide with byes) and the runs for each type present | —                                                                                                                                                        | Runs conceded, balls faced (a no-ball counts as faced, a wide does not), team total                                                  |
...
| Runs conceded                                           | Participant (bowling), fixture  | Delivery, Extra                                          | Runs off bat + wides + no-balls, excluding byes and leg-byes                                                                           | Basic                                                                                                  |
...
| Super-over statistics                                   | Fixture, participant            | Delivery (super-over innings)                            | Same calculations as standard statistics, but computed on a separately scoped set; **excluded from standard aggregates by default**    | Intermediate/Advanced boundary — convention still to be confirmed with the client (open question, §11) |
...
2. **Extras types can co-occur** on one delivery (e.g. wide + byes).
