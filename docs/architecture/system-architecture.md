...
The following issue #55 records define compatible foundations for later-tier work. Issue #356
accepted ADR-010 and ADR-011 for Intermediate implementation. ADR-009 and ADR-012 remain proposed.
An accepted decision selects a target; it does not mean that a service is provisioned or a feature
is implemented.

| Concern                   | Decision direction                                                                                                                                                   | Record                                                                                                                                                |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Worker and job queue                    | Use a PostgreSQL job/outbox transaction, Azure Service Bus Standard, and a separate idempotent Node worker for large imports, recomputation, exports, and scheduled synchronisation.                                                            | Intermediate target accepted under #356; provisioning and implementation remain follow-on work.                                                                                                      |
...
- verify frontend and backend deployment workflows;
...
| Object storage and retention                      | Storing large source/export bytes in PostgreSQL raises cost; unmanaged files raise security, privacy, and deletion risks.                                                                                                          | Accepted ADR-011 uses private Azure Blob Storage with PostgreSQL provenance and 90-day batch-source retention. Verify scanning, residency, licence, recovery, and cost during provisioning.                                            | Before large/public uploads or exports.                                                               |
...
| Queue/cache consistency                           | A lost job or stale cache can publish statistics from superseded events.                                                                                                                                                           | Apply accepted ADR-010 for jobs and proposed ADR-009 only after its cache adoption gate: use atomic outbox/job creation, idempotent workers, versioned cache keys, invalidation tests, and reconciliation.                             | Before Intermediate rollout.                                                                          |
